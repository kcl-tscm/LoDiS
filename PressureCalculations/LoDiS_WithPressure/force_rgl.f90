...
!
  ...
  REAL(8) :: x1cdm, z2cdm
  REAL(8) :: presbi, presri, prsr, prsb
...
!! MODIFIED
! PRESSURE
! Initialize the cumulative pressure array at 0. It will be incrementally summed at each time step
...
!                                              
	...
	! Pressure
	   prsr=p(itypik)*a(itypik)*pexp/dik0
           prsb=-q(itypik)*qsi2*qqexp/dik0
        ELSE
!
! analytical extention of the potential up to cutoff_end
!
           dikm=dik-cutoff_end 
           !
           dikm2=dikm*dikm
...
&4.d0*x4(itypik)*dikm3+3.d0*x3(itypik)*dikm2)      
	...
	presbi=presbi+prsb
     ...
     ! PRESSURE
     presbi=den(i)*presbi
	! Modify array of pressure of all atoms
     pressure_i(i)=presri+presbi !Total pressure is composed of repulsion and bonding
